...
class User extends Base\User
{
    ...
    {
        $q = ORM::select('Bazalt\Login\Model\User u')
                ->leftJoin('Bazalt\Login\Model\SiteRefUser ref', array('user_id', 'u.id'))
    ...
    {
        $sid = \Framework\System\Session\Session::getSessionId();
        return md5($this->id . $sid . CMS\Bazalt::getSecretKey() . time());
    }
    ...
    {
        $q = ORM::select('Bazalt\Login\Model\User u')
                ->innerJoin('Bazalt\Login\Model\UserSetting ref', array('user_id', 'u.id'))
    ...
    public function updateLastActivity($time = null)
    {
        ...
        ORM::update('Bazalt\Login\Model\User')
    ...
    public static function deleteByIds($ids)
    {
        ...
        $q = ORM::delete('Bazalt\Login\Model\User a')
                ->whereIn('a.id', $ids)
    ...
    {
        $q = ORM::select('Bazalt\Login\Model\User f')
